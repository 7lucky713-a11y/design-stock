...
import { ArrowUpRight, BookmarkPlus, Check, ChevronRight, ImagePlus, Search, Sparkles, Trash2, X } from 'lucide-react';
...
export default function DesignStockApp() {
  ...
  function addFromSource(source: Source) {
    setPrefill(source);
    setOpenForm(true);
  }
  ...
  return (
    <main className="shell">
      <header className="topbar">
        ...
        <button className="primary" onClick={() => { setPrefill(null); setOpenForm(true); }}><BookmarkPlus size={18}/> ストックを追加</button>
      </header>
      ...
      {tab === 'stock' ? (
        <>
          ...
          {loading ? <p className="empty">読み込み中…</p> : visible.length === 0 ? (
            <div className="emptyState"><Sparkles size={28}/><h2>まだストックがありません</h2><p>参考サイトを見つけたら、URLと「どこが良かったか」を残してみよう。</p><button className="primary" onClick={() => setOpenForm(true)}>最初の1件を追加</button></div>
          ) : (
            <div className="masonry">
              {visible.map(stock => <article className="card" key={stock.id}>
                ...
                <div className="cardBody">
                  ...
                  <button className="delete" onClick={() => removeStock(stock.id)}><Trash2 size={15}/> 削除</button>
                </div>
              </article>)}
            </div>
          )}
        ...
        <section className="sourceGrid">
          {sources.map((source, i) => <article className="sourceCard" key={source.id}>
            ...
            <div className="sourceActions"><a href={source.url} target="_blank" rel="noreferrer">見に行く <ArrowUpRight size={15}/></a><button onClick={() => addFromSource(source)}>ここからストック</button></div>
          </article>)}
        </section>
      )}

      {openForm && <StockForm prefill={prefill} onClose={() => setOpenForm(false)} onSaved={(stock) => { setStocks(prev => [stock, ...prev]); setOpenForm(false); setTab('stock'); }} />}
    </main>
  );
}

function StockForm({ prefill, onClose, onSaved }: { prefill: Partial<Source> | null; onClose: () => void; onSaved: (stock: Stock) => void }) {
  const [title, setTitle] = useState(prefill?.name ?? '');
  const [url, setUrl] = useState(prefill?.url ?? '');
  const [focusNote, setFocusNote] = useState('');
  const [memo, setMemo] = useState('');
  const [tagText, setTagText] = useState(prefill?.strengths?.join(', ') ?? '');
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/stocks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, url, focusNote, memo, tags, imageKey, imageUrl }) });
      if (!res.ok) throw new Error('ストックの保存に失敗しました');
  ...
  return <div className="modalBackdrop" onMouseDown={onClose}><div className="modal" onMouseDown={e => e.stopPropagation()}>
    <div className="modalHead"><div><p className="eyebrow">NEW REFERENCE</p><h2>参考ポイントを保存</h2></div><button className="iconButton" onClick={onClose}><X/></button></div>
    <form onSubmit={submit}>
      ...
      <label className="upload"><input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={e => setFile(e.target.files?.[0] ?? null)} /><ImagePlus size={23}/><span>{file ? file.name : '参考箇所のスクショを追加'}</span><small>画像はR2へ保存</small></label>
      {error && <p className="error">{error}</p>}
      <button className="primary save" disabled={saving}>{saving ? '保存中…' : <><Check size={18}/> STOCKする</>}</button>
    </form>
  </div></div>;
}
